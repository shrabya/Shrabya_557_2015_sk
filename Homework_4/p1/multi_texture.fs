...
uniform sampler2D texture_newground; //this is the texture
...
void main(void)                                                   
{
    ...
    vec4 tex_color_light_new =  texture(texture_newground, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + (tex_color + tex_color_light + tex_color_light_new)/2 ;
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light* tex_color_light_new;
    }
    else if(texture_blend == 2)
    {
        color = (tex_color_light.r)*tex_color* tex_color_light_new ;
    }
    else
    {
